...
def refresh_token_if_needed():
    ...
    if time.time() > TOKEN_EXPIRY_TIME:
        ...
        try:
            ...
            print(f"[HATA] Token yenilenemedi: {e}")
            return False
    return True
...
def get_all_songs():
    ...
    try:
        ...
        return [value['track'] for value in all_tracks_data.values() if isinstance(value, dict) and 'track' in value]
    except requests.exceptions.RequestException as e:
...
def get_account_names(account_ids):
    ...
    try:
        ...
        for i in range(0, len(unique_ids), 100):
            ...
            if i + 100 < len(unique_ids): time.sleep(1) 
...
def parse_entry(raw_entry, account_id):
    """API'den gelen veriyi işler ve best_run'ı bulur. Account ID ekler."""
    best_score = -1
    best_run_stats = None
    ...
    if best_run_stats:
        return {
            "account_id": account_id, # ARTIK ID KAYDEDİLİYOR
    ...
    for i, song in enumerate(all_songs):
        ...
        for page_num in range(PAGES_TO_SCAN):
            try:
                ...
                if response.status_code == 404: sys.stdout.write('\n'); break 
                response.raise_for_status()
                raw_entries = response.json().get('entries', [])
                if not raw_entries: sys.stdout.write('\n'); break
                ...
                unique_entries_map = {} # { 'account_id': entry_data }

                for entry in raw_entries:
                    acc_id = entry['teamId']
                    parsed_entry = parse_entry(entry, acc_id)
                    
                    if parsed_entry:
                        parsed_entry['userName'] = user_names.get(acc_id, "Unknown User")
                        
                        # Eğer ID zaten varsa, sadece daha yüksek skor ise güncelle
                        if acc_id in unique_entries_map:
                            if parsed_entry['score'] > unique_entries_map[acc_id]['score']:
                                unique_entries_map[acc_id] = parsed_entry
                        else:
                            unique_entries_map[acc_id] = parsed_entry

                parsed_data = {'entries': list(unique_entries_map.values())}
                # -----------------------------
...
if __name__ == "__main__":
    if not EPIC_REFRESH_TOKEN or not EPIC_BASIC_AUTH:
        print("[HATA] Secret'lar eksik."); sys.exit(1)
        
    if len(sys.argv) < 2:
        print("Kullanım: python actions.py [enstrüman] [klasör]"); sys.exit(1)
